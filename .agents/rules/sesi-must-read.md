...
7. **Emphasize Native Verification Commands:** Prior to saving or running full Sesi scripts, proactively use inline evaluation (`npm run eval <file>.sesi` or `sesi -e "..."`) to check and verify syntax and runtime behaviors instantly. It keeps execution cycles fast and deterministic.
...
For all quirks and specific syntaxing, visit GUIDE.md, dist/, node_modules/@misterscan/sesi/docs/WRITING_SCRIPTS.md, and getting-started/ to verify against the actual engine specifications. Do not rely on training data or outdated documentation for syntax rules, always verify against the source.
